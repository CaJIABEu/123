...
function bots(VK, token) {  
	...
	vk.setOptions({
		token: '9553712545cb6b291df3e0ef5ee65727cbb04633447136d5d725bb24c3a2941ea8c3dead60fefe54a012a',
		apiMode: 'parallel',
		pollingGroupId: 192667743
	});  
	/*==========================================================================================================*/
	...
	*/
	...
	vk.updates.hear(/^(?:cmd|команды|кмд|помощь|✅ Помощь|@privat_project ✅ Помощь)/i, (message) => { 
		...
		if(aid != 1){
			return message.send(`
				...
				‼ Только в основной группе: vk.com/imperialbot
				- - - - - -  
	...
	vk.updates.hear(/^(?:альянс инфо)$/i, (message) => {  
		return message.send(`
			...
			Чтобы создать Альянс, нужно написать в ЛС ГРУППЫ 
			- - -
			"Создать [TOKEN] [ID]"
	...
	vk.updates.hear(/^(?:вип)/i, (message) => { 
		return message.send(`
			...
			‼ Отправив сумму, указав idcajiabeu.
		`);
	});
	/*==========================================================================================================*/ 	
	...
	/*==========================================================================================================*/
 	...
 	vk.updates.hear(/^(?:бот)/i, (message) => {
 		return message.send(`
 		&#129302; Проект: [imperialbot|Империал Бот] 
		Империал Бот - это принципиально новый бот от всеми известного создателя, из под чьих ловких рук вышло не мало легенд этой отрасли! В этом проекте были учтены недостатки прошлых и мы представляем вам увлекательную РПГ игру с невероятным количеством общения, сражений, строительства, развития! 
 		...
 		&#128128; Developer(coder): [c_o_d_e_r|Vlad Kucher]

 		&#128122; Дизайнер: [brainbooom|Гриша]

 		👻 Креативщик: [laribob|Dimon]
